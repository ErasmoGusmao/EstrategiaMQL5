...
if(comprado == false) //Travo o sistema para enviar uma única ordem
  ...
  comprado = true;
   ...
   //Enviar várias oredens
   for(int i=0;i<nOrdens;i++)
     {
     
      double preco2 = (Preco +(i*0.01));
      //Enviar ordem pendente
      MqlTradeRequest requisicao;
      ZeroMemory(requisicao);  //Sempre tem que zerar a requisição
      requisicao.action = TRADE_ACTION_PENDING; //Coloca um ordem de negociação para execução sob condições específicas (ordem pendente)
      requisicao.symbol = Symbol();
      requisicao.volume = Quantidade;
      requisicao.price = preco2;
