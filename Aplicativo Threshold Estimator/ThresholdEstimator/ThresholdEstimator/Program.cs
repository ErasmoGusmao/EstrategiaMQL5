...
namespace ThresholdEstimator
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {    
                writer.WriteLine("Cenário\tMA(10)\tMA(15)\tMA(20)\tMACD\tRSI\tTotal");
                        ...
                        for (int k = 0; k < força_padrão_MA_20.Length; k++)
                        {
                            for (int m = 0; m < força_padrão_MACD.Length; m++)
                            {
                                for (int n = 0; n < força_padrão_RSI.Length; n++)
                                {
                                    ...
                                    SinalFinal = (SinalMA_10 + SinalMA_15 + SinalMA_20 + SinalMACD + SinalRSI) / 5;

                                    Console.WriteLine("{0}. MA(10): {1}\t MA(15): {2}\t MA(20): {3}\t MACD: {4}\t RSI: {5}\t Total: {6}",
                                        cont.ToString(), SinalMA_10.ToString(), SinalMA_15.ToString(), SinalMA_20.ToString(),
                                        SinalMACD.ToString(), SinalRSI.ToString(), SinalFinal.ToString());
                                    //Escrever no .txt
