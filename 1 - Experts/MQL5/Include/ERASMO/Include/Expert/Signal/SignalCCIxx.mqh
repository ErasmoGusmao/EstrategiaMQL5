...
//| Class=CSignalCCIxx                                               |
//| Page=signal_CCIxx                                                |
//| Parameter=PeriodCCIxx,int,8,Period of calculation                |
//| Parameter=Applied,ENUM_APPLIED_PRICE,PRICE_CLOSE,Prices series   |
//| ATENÇÃO: ESSA CLASSE É UMA MODIFICAÇÃO DA CLASSE PADÃO SignalCCI |
//|         AQUI ELA FOI MODIFICADA PARA FINS DIDATICOS COM A CRIAÇÃO|
//|         DE DOIS NOVOS SINAIS                                     |
//|         TUDO QUE SE REFERIA A CCI FOI SUBISTITUIDO POR CCIxx     |
//|      MENOS A CLASSE CiCCI QUE INSTANCIA m_CCIxx (OBJETOOSCILADOR)|
//+------------------------------------------------------------------+
...
protected:
   ...
   //--- adjusted parameters
   int               m_periodCCIxx;      // the "period of calculation" parameter of the oscillator
   ...
   //============ MEUS NOVOS PADRÕES DE SINAL
   int               m_pattern_4;      // modelo 4 " 1o novo padrão: Valores que cruzão o zero"
   int               m_pattern_5;      // modelo 5 " 2o novo padrão: Valores que varião entorno do zero"
   //========================================
   //--- variables
...
public:
   ...
   void              PeriodCCIxx(int value)              { m_periodCCIxx=value;           }
...
protected:
   ...
   double            CCIxx(int ind)                      { return(m_CCIxx.Main(ind));     }
   double            Diff(int ind)                     { return(CCIxx(ind)-CCIxx(ind+1)); }
   int               State(int ind);
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CSignalCCIxx::CSignalCCIxx(void) : m_periodCCIxx(14),
                               ...
                               m_pattern_3(50),
              //============ MEUS NOVOS PADRÕES DE SINAL
                               m_pattern_4(90),       //Peso do meu modelo 4 no sinal do CCI
                               m_pattern_5(90)       //Peso do meu modelo 5 no sinal do CCI
              //=========================================                 
  {
...
//| Destructor                                                       |
//+------------------------------------------------------------------+
CSignalCCIxx::~CSignalCCIxx(void)
  {
  }
//+------------------------------------------------------------------+
//| Validation settings protected data.                              |
//+------------------------------------------------------------------+
bool CSignalCCIxx::ValidationSettings(void)
  {
...
//--- initial data checks
   if(m_periodCCIxx<=0)
     {
      printf(__FUNCTION__+": period of the CCIxx oscillator must be greater than 0");
      return(false);
     }
//--- ok
   return(true);
...
//| Create indicators.                                               |
//+------------------------------------------------------------------+
bool CSignalCCIxx::InitIndicators(CIndicators *indicators)
  {
//--- check pointer
   if(indicators==NULL)
      return(false);
//--- initialization of indicators and timeseries of additional filters
   if(!CExpertSignal::InitIndicators(indicators))
      return(false);
//--- create and initialize CCIxx oscillator
...
//+------------------------------------------------------------------+
//| Initialize CCIxx oscillators.                                      |
//+------------------------------------------------------------------+
bool CSignalCCIxx::InitStoch(CIndicators *indicators)
  {
//--- check pointer
   if(indicators==NULL)
      return(false);
//--- add object to collection
   if(!indicators.Add(GetPointer(m_CCIxx)))
     {
      printf(__FUNCTION__+": error adding object");
      return(false);
     }
//--- initialize object
   if(!m_CCIxx.Create(m_symbol.Name(),m_period,m_periodCCIxx,m_applied))
     {
...
//+------------------------------------------------------------------+
int CSignalCCIxx::State(int ind)
  {
   int    res=0;
   double var;
//---
...
//+------------------------------------------------------------------+
bool CSignalCCIxx::ExtState(int ind)
  {
...
//+------------------------------------------------------------------+
bool CSignalCCIxx::CompareMaps(int map,int count,bool minimax,int start)
  {
...
//+------------------------------------------------------------------+
int CSignalCCIxx::LongCondition(void)
  {
   int result=0;
   int idx   =StartIndex();
//---
   if(Diff(idx)>0.0)
     {
      ...
      if(IS_PATTERN_USAGE(2) || IS_PATTERN_USAGE(3))
        ...
        // Modelo 4 usado para procurar o cruzamento da linha do zero
      if(IS_PATTERN_USAGE(4) && CCIxx(idx+2)<0.0 && CCIxx(idx+1)>0.0)
        {
         return(m_pattern_4);    //sinal de número 4
        }
      if(IS_PATTERN_USAGE(5) && CCIxx(idx+3)>0.0 && CCIxx(idx+2)<0.0 && CCIxx(idx+1)>0.0)
        {
         return(m_pattern_5);    //sinal de número 5
        }    
     }
//--- return the result
   return(result);
...
//+------------------------------------------------------------------+
int CSignalCCIxx::ShortCondition(void)
  {
   int result=0;
   int idx   =StartIndex();
//---
